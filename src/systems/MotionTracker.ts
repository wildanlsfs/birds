      ...
      try {
          ...
          const loadPromise = (async () => {
                    // 1. Dynamic import of tasks-vision (keeps initial bundle ultra-light)
                                       ...
                                       // Use Vite's BASE_URL so this resolves correctly under a subpath
                                       // (e.g. GitHub Pages project sites served from /birds/) instead of
                                       // always hitting the domain root.
                                       const localWasmDir = `${import.meta.env.BASE_URL}mediapipe/wasm`;
                    let vision;
                    try {
                                vision = await FilesetResolver.forVisionTasks(localWasmDir);
                    } catch (localWasmErr) {
                                ...
                                vision = await FilesetResolver.forVisionTasks(
                                       ...
                                       const localModelPath = `${import.meta.env.BASE_URL}mediapipe/models/pose_landmarker_lite.task`;
